...
import { ResultContainer } from './styles/GameBot'
  ...
  return (
    <div className={`flex flex-col ${isLast ? 'flex-grow' : ''}`}>
      <div className="flex mb-4">
        <div className="rounded mr-4 h-full w-32 md-w-10 relative overflow-hidden">
          <Image
        ...
        {message.html ? (
          <ResultContainer
            className={`${message.type === 'user' ? 'user' : 'bot'} whitespace-pre-line	message-content`}
            style={{ maxWidth: '90%' }}
            dangerouslySetInnerHTML={{ __html: message.html }}
          />
        ) : (
          <p className={`${message.type === 'user' ? 'user' : 'bot'} whitespace-pre-line	`} style={{ maxWidth: '90%' }}>
            {message.text}
          </p>
        )}
      </div>
